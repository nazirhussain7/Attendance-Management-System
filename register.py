...
def database():
    ...
    if name1=="":
        messagebox.showinfo("Attendance","Enter Name")
    else:
        if email == "":
            messagebox.showinfo("Attendance","Enter Email")
        else:
            if contact == "":
                messagebox.showinfo("Attendance", "Enter Contact")
            else:
                if age == "":
                    messagebox.showinfo("Attendance", "Enter Course")
                else:
                    if un == "":
                        messagebox.showinfo("Attendance", "Username")
                    else:
                        if pw == "":
                         messagebox.showinfo("Attendance", "Enter Password")
                        else:
                            if not (re.search(regex, email)):
                                messagebox.showinfo("Attendance", "Enter valid Email")
                            else:
                                if l != 10:
                                    messagebox.showinfo("Attendance", "Enter 10 digits only")
                                else:
                                    if  not name1.isalpha():
                                        messagebox.showinfo("Attendance", "Enter Name in alphabets Only ")
                                    else:
                                                ...
                                                with conn:
                                                    ...
                                                    if dbuser == name1 and email == ph1:
                                                        messagebox.showinfo("Att", "Already Registered")
                                                    else:
                                                        cursor.execute(
                                                        ...
                                                        conn.commit()
                                                        messagebox.showinfo("Attendance","Record Saved")
...
label_0 = Label(root, justify=LEFT, bg='white', text="Registration Here..", width=50, font=("bold", 20))
label_0.place(x=300, y=230)
...
label_4 = Label(root, text="College", bg='white',justify=LEFT, width=20, font=("bold", 10))
...
entry_6.place(x=450, y=500)

label_7 = Label(root, text="Course",bg='white', justify=LEFT, width=20, font=("bold", 10))
label_7.place(x=300, y=550)

entry_7 = Entry(root, textvar=Age)
entry_7.place(x=450, y=550)

label_8 = Label(root, text="Semester",bg='white', justify=LEFT, width=20, font=("bold", 10))
...
Button(root, text='Submit', width=10, bg='gray', fg='white', command=database).place(x=850, y=650)
